...
namespace UnityStandardAssets.Vehicles.Car
{
	...
	public class WaypointTracker_pid
	{
		...
		public float AngleToNextWaypoint(CarController cc)
		{
			Vector3 p = cc.transform.position;
			float closestLen = 100000; // large number
			int closestWaypoint = 0;
			...
			Vector3 heading = waypoints[closestWaypoint] - p;
			heading.y = 0; // Ignore vertical component for 2D angle calculation
			float angle = Quaternion.Angle(cc.transform.rotation, Quaternion.LookRotation(heading));

			if (angle > 90)
			{
				closestWaypoint = (closestWaypoint + 1) % waypoints.Count;
				heading = waypoints[closestWaypoint] - p;
				heading.y = 0;
				angle = Quaternion.Angle(cc.transform.rotation, Quaternion.LookRotation(heading));
			}

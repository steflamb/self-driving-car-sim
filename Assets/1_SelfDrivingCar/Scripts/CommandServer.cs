...
public class CommandServer : MonoBehaviour
{
	...
	void OnWeather (SocketIOEvent obj)
	{
		SceneManager.LoadScene("GeneratedTrack");
		JSONObject jsonObject = obj.data;
		string weather = jsonObject.GetField("type").str;
		int rate = int.Parse (jsonObject.GetField("rate").str);
		Debug.Log ("Requested weather: " + weather + "rate: " + rate);
		WeatherController.setWeather (weather, "Constant", rate);
	...
	void EmitTelemetry ()
	{
		...
		UnityMainThreadDispatcher.Instance ().Enqueue (() => {
			...
			if ((Input.GetKey (KeyCode.W)) || (Input.GetKey (KeyCode.S))) {
				...
				data["angle5"] = _carController.transform.rotation.eulerAngles[0].ToString("N4");
				data["angle6"] = _carController.transform.rotation.eulerAngles[1].ToString("N4");
				data["angle7"] = _carController.transform.rotation.eulerAngles[2].ToString("N4");
				data ["steering_angle"] = _carController.CurrentSteerAngle.ToString ("N4");
				data ["throttle"] = _carController.AccelInput.ToString ("N4");
				data ["speed"] = _carController.CurrentSpeed.ToString ("N4");
				data["hit"] = _carController.GetLastCollision();
				_carController.ClearLastCollision();
				Debug.Log("Rot: " + _carController.transform.rotation);
				...
				if (pm != null)
                {
                    ...
                    Transform tm = _carController.transform;
					// Vector3 carPosition;
					// ref Vector3 lookAheadPoint;
					// float carOrientation;
                    float cte = 0.0f;
					float angl = 0.0f;
					// ref Vector3 carPosition, ref float carOrientation, ref Vector3 lookAheadPoint
                    (bool, bool) cte_ret = pm.carPath.GetCrossTrackErr(tm, ref cte, ref angl);
					// (bool, bool) cte_ret2 = pm.carPath.GetCrossTrackErrExtended(tm, ref cte, ref angl, ref carPosition, ref carOrientation, ref lookAheadPoint);
					...
					data["cte_pid"] = cte_pid.ToString("N4");

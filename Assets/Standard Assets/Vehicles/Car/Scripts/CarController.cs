	...
	public class CarController : MonoBehaviour
	{
		...
		public IEnumerator WriteSamplesToDisk ()
		{
			...
			if (carSamples.Count > 0) {
				...
				// physically moving the car to get the right camera position
		...
		public IEnumerator Sample ()
		{
			...
			if (m_saveLocation != "") {
				...
				sample.cte = tracker.CrossTrackError (this);
				sample.lap = manager.getLapNumber ();
				sample.sector = manager.getCurrentWayPointNumber ();
				// sample.heading_diff = tracker.AngleToNextWaypoint (this);
